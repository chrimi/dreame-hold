...
class DreameHoldWeekdaySwitch(DreameHoldEntity, SwitchEntity):
    ...
    `async_update_property_atomic`, which serializes concurrent toggles
    without needing a fresh device read before every single one (see that
    method's docstring: a confirmed live "connection lost" bug when
    several of these were toggled in quick succession). This also
    re-derives the mask's "one-time" flag from the resulting day
    ...
    async def _set(self, enabled: bool) -> None:
        def mutate(current: int) -> int:
            ...
            # Derive the one-time flag from the resulting day selection
            # rather than preserving whatever it was before this toggle -
            # a schedule can't be both "one-time" and repeat on specific
            # days. Turning a day on always makes this a repeating
            # schedule; turning the last remaining day off correctly
            # falls back to "one-time" (see derive_one_time_flag's
            # docstring for the bug this fixes).
            return encode_weekday_mask(decoded, one_time=derive_one_time_flag(decoded))
