"""Base authentication class for Dreame/MOVA cloud services.

Adapted from antondaubert/dreame-mower's dreame/cloud/cloud_base.py (MIT
license), which itself descends from Tasshack/dreame-vacuum's protocol.py.
Renamed from DreameMowerCloudBase -> DreameCloudBase; behavior unchanged.
            ...
            try:
                ...
                if self.__http_api_connected:
                    if isinstance(ex, requests.exceptions.Timeout):
                        _LOGGER.warning(
                            "DreameCloudBase.request: Read timed out. (read timeout=%s) for URL %s: %s",
                            timeout,
                            url,
                            data,
                        )
